...
In benchmarked case studies spanning immunology, neurology, and other disease areas, the causal gene was recovered in 85.6\% of all returned panels, with the subjective user-selected ``best panel'' achieving a perfect accuracy of 100\%. The underlying dataset includes standardised gene identifiers, disease annotations, MOI, and literature support, enabling seamless integration into bioinformatic pipelines. 
\vspace{1em}
...
\noindent
To mimic a clinician who is trying to diagnose a genetic disease, we systematically selected genetic diagnosis case reports that used exome or genome sequencing. We chose the first five eligible case reports from each of three sources: \ac{jaci}, the journal Neurology, and for random cross-disciplinary topics we used pre-defined PubMed search restricted to the last 12 months and free full text, yielding a total of 15 case studies across phenotypes.  This selection allowed us to validate the method across immunological, neurological, and other disease areas.

For each case report, we used only the clinical narrative before any genetic analysis was introduced in the original publication. We used a standardised method to select the phenotype terms from each case report and then rank by most to least importance or relevance. The ranked phenotype terms were then used a keyword queries, simulating a naïve starting position for a clinician. Search terms were added incrementally until no panels matched. We then tested whether our PanelAppRex tool could successfully retrieve panels that included the final causal gene reported in each case study. The sources, queries, and results are shown in \textbf{Table \ref{tab:benchmark}}. Benchmarking analyses were performed on the core dataset; the AI-derived demonstration ``Info'' field described below was not used in this evaluation.
...
\noindent
% To assess whether the dataset was analysis-ready, we audited completeness of key annotation fields required for joining, filtering, and interpretation (\textbf{Figure \ref{fig:validation}}). For publications, disease panel names, \ac{moi}, and OMIM gene identifiers, we report entry-level completeness across all gene-panel entries. For HGNC and Ensembl gene identifiers, we used automated recovery of missing IDs from Ensembl as it is not a subjective decision. After recovery, the core identifiers required for gene-panel linking were present at 100\% completeness. A small number of entries are incomplete for Ensembl ID such as \textit{HPE1} because it is a phenotype identifier rather than gene name, and is thus not present on HGNC. Some secondary annotation fields remained incomplete such as \ac{moi}, and we did not attempt to fill them in the core dataset because this would require non-primary-source decisions which are better reserved for independent downstream implementations. The validated dataset therefore provides a stable and analysis-ready identifier layer for downstream integration (\textbf{Figure \ref{fig:validation}}).
...
We report entry-level completeness across all gene-disease panel entries. For HGNC and Ensembl gene identifiers, we additionally used automated recovery of missing IDs from Ensembl because these identifier mappings are not subjective. After recovery, the core identifiers required for gene-panel linking were present at 100\% completeness. A small number of entries are incomplete for Ensembl ID such as \textit{HPE1} because it is a phenotype identifier rather than gene name, and is thus not present on HGNC. Some secondary annotation fields remained incomplete, including \ac{moi}, and we did not attempt to fill these in the core dataset because doing so would require non-primary-source decisions better reserved for downstream implementations. The validated dataset therefore provides a stable and analysis-ready identifier layer for downstream integration while preserving the original availability of secondary annotations.
...
Notably, for all case studies, all correct causal genes were successfully retrieved by the system despite never appearing explicitly in any of our submitted query terms (\textbf{Table \ref{tab:benchmark}}).
In our evaluation, PanelAppRex returned panels in which the causal gene was present in 
% 93\% - immunology only in version 1
85.6\% of all returned panels, with the subjective user-selected ``best panel'' achieving a perfect accuracy of 100\% 
...
\subsection{Applications in AI-based RAG}
\noindent
Across all panels, we used the core PanelAppRex dataset to generate AI-derived panel summaries that captured panel-specific mechanisms and phenotypes conditioned on disease context and gene composition. 
...
\section{Discussion}
\noindent
PanelAppRex provides an accessible and validated platform for querying, aggregating, and exporting curated disease-gene panel data. Designed for both clinical and research use, it simplifies study planning and variant interpretation through a user-friendly interface, while also offering a harmonised dataset for programmatic workflows. 
The core model integrates over 58,000 panel-gene associations, with complete coverage of the core gene identifiers required for gene-panel linking after recovery, alongside structured disease annotations, \ac{hgnc} symbols, \ac{moi}, OMIM-linked identifiers, and supporting publications where available.
...
We performed benchmarks to measure success for both relatively simple gene-phenotype relationships (i.e., \ac{pid}) and more complex relationships (i.e., neurogenetic disorders) but ultimately, a measure of long-term public option will be required to determine the practical benefit.
...
% However, the query was still considered 100\% successful, as the causal gene appeared in the combined result set, consistent with the recommended bioinformatic strategy of using the union of returned panels.
We note that a pre-compiled offline database is useful for secured computing environments but it misses out on the benefits of well maintained primary source APIs: for rapidly changing resources, direct API access can provide more frequent updates than an offline snapshot, which we cannot match in the same way.
...
Case study 5 had five individual cases and patient information was significantly longer than other studies. Therefore, we used OpenAI model o3-mini to converted each to a standardised keyword query automatically, thereby removing our subjective bias and aligning with the other queries.}
\label{tab:benchmark}
%\centering
%\resizebox{\textwidth}{!}{
%\begin{tabular}{|
...
\caption{\textbf{Validation of completeness major annotation fields in the core dataset.}
...
For all panels the percentages are calculated relative to total gene-panel entries, except for panels A and B where the additional ``unique genes'' bars are calculated relative to the total number of unique genes.
}
...
\noindent
To assess whether PanelAppRex improves practical query performance beyond direct use of an upstream source, we performed a cold-start benchmark against a best-case \ac{ge} PanelApp \ac{api} workflow by re-using the first validation case query, ``\textit{SERPING1}, Factor XII, and edema'' (\textbf{Figure \ref{fig:benchmark_cold_start}}). We used the same query in both settings. This comparison is not fully like-for-like, because the \ac{ge} PanelApp \ac{api} does not accept the same phenotype-style query and return a ranked panel result directly. Instead, panel data must first be retrieved and assembled before ranking can begin. To make the comparison as favourable as possible to the \ac{api} workflow, we deliberately included the known relevant panel from validation case study 1 and then increased the number of queried panels. In practice, a user would not usually know that panel in advance.

PanelAppRex local was faster at every tested subset size for time to final ranked result. The \ac{api} workflow also required increasing numbers of panel-level requests in order to assemble the returned data before ranking could begin. After 150 queried panels, the \ac{ge} \ac{api} returned HTTP 429 rate-limiting errors, so the full panel set could not be evaluated beyond in this setting without adding a time delay. In addition, the \ac{api} comparator returned only panel content, whereas PanelAppRex provided the pre-integrated local dataset together with the annotations used elsewhere in the platform such as gene function and disease details.
