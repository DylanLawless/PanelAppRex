...
# <iframe height="400" width="100%" frameborder="no" src="https://switzerlandomics.shinyapps.io/query_app/"> </iframe>
...
server <- function(input, output, session) {
  output$small_table <- renderDT({
    datatable(
      ...
      options = list(
        ...
        columnDefs = list(
          list(
            targets = 3,    # zero-based index for 'entity_names' column
            visible = FALSE,
            searchable = TRUE
          )
        )
  ...
  filtered_big <- reactive({
    req(selected_ids())
    df_core %>% filter(panel_id %in% selected_ids())
  })
